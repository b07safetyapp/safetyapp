...
public class TEMPQuestionnaireActivity extends AppCompatActivity { //11. Activity class

    private TEMPQuestionnaireViewModel viewModel;           //12. Reference to ViewModel
    private TEMPQuestionAdapter adapter;                   //13. Adapter instance
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) { //15. onCreate
        ...
        viewModel = new ViewModelProvider(this).get(TEMPQuestionnaireViewModel.class);
        ...
        viewModel.getQuestions().observe(this, questions -> {
            if (adapter == null) {                     //24. First time setup
                adapter = new TEMPQuestionAdapter(questions,
